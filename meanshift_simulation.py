from utils.simulation import gen_mean_shift_P, gen_mean_shift_Q, compute_arl_edd
...
def run_nncusum_simulation(data_dim: int, hidden_dims: list, window_size: int, stride: int, 
    ...
    for i in tqdm(range(iter_num)):
        ...
        # generate data
        X = gen_mean_shift_P(data_dim, pilot_length)
        Ynull = gen_mean_shift_P(data_dim, cp_location+burnin_length)
        Y = gen_mean_shift_Q(data_dim, postcp_length, 0.1)
    ...
    np.save(os.path.join(save_dir, 
                         "nncusum_meanshift_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_s{}_w{}".format(data_dim, iter_num, 
                            cp_location, postcp_length, burnin_length,  
                            hidden_dims[0], len(hidden_dims), stride, window_size)), stat_record)
    save_data(
        os.path.join(
            save_dir, 
            "nncusum_meanshift_arledd_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_s{}_w{}.pkl".format(
            data_dim, iter_num, 
            cp_location, postcp_length, burnin_length,  
...
def run_onnc_simulation(data_dim: int, hidden_dims: list, window_size: int, stride: int, learning_rate: float, epoch: int,
    ...
    np.save(
        os.path.join(
            save_dir,
            "onnc_meanshift_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_s{}_w{}_e{}".format(
                data_dim,
                iter_num,
                cp_location,
                postcp_length,
    ...
    save_data(
        os.path.join(
            save_dir, 
            "onnc_meanshift_arledd_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_s{}_w{}_e{}.pkl".format(
                data_dim,
                iter_num,
                cp_location,
                postcp_length,
...
def run_onnr_simulation(data_dim: int, hidden_dims: list, window_size: int, alpha:float, stride: int, learning_rate: float, 
    ...
    np.save(
        os.path.join(
            save_dir,
            "onnr_meanshift_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_a{}_s{}_w{}_e{}".format(
                data_dim,
                iter_num,
                cp_location,
                postcp_length,
    ...
    save_data(
        os.path.join(
            save_dir, 
            "onnr_meanshift_arledd_dim{}_iter{}_pre{}_post{}_b{}_h{}_l{}_s{}_w{}_e{}.pkl".format(
                data_dim,
                iter_num,
                cp_location,
                postcp_length,
