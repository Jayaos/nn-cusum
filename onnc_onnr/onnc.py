from onnc_onnr.online_change_clf import ChangePointDetectionOnline
...
def run_onnc(hidden_dims: list, window_size: int, stride: int, lag_size: int,
    ...
    for i in tqdm(range(iter_num)):
        ...
        clf = ChangePointDetectionOnline(metric="KL_sym", periods=1, batch_size=window_size,
                                         lag_size=lag_size, step=stride, n_epochs=epoch, lr=learning_rate, 
                                         lam=0.0, optimizer="Adam", device=device)

        sequence = np.concatenate([x_chunk, y_chunk])
        score_clf, peaks_clf = clf.predict(hidden_dims, sequence, height=1, smooth=False)      
        stat_record[i,:] = score_clf[:-shift_size]

    print("saving results...")
    os.makedirs(save_dir, exist_ok=True)
    np.save(os.path.join(save_dir, 
                         "onnc_iter{}_pre{}_post{}_d{}_l{}_s{}_w{}_e{}".format(iter_num, f0_length, f1_length,
